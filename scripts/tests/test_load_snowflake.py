"""
Test Suite for the load_snowflake module. This test verifies that
the main function in tha module correctly proccesses the given
JSON data, mimicking what would be provided by the upstream data
ingestion and processing pipeline.

Monkeypatch and mock API calls are uses to avoid uneccesary API usage
or network issues.
"""
...
def test_load_snowflake_pipeline_ingestion_loop(monkeypatch, capsys): # pylint: disable=unused-argument
    ...
    for query in insert_queries:
        ...
        assert "%s" in query and "f" not in query, "Security Violation: Hard-coded formatting detected." \
